...
def _make_dinov3_vit(
    ...
    **kwargs,
    ...
    if pretrained:
        ...
        # PROMPT-TOKEN EXTENSION: for linear probe, remove the prompt-token path and keep strict=True loading.
        model.init_weights()
        if getattr(model, "n_prompt_tokens", 0) > 0:
            # PROMPT-TOKEN EXTENSION: keep strict=False only when n_prompt_tokens > 0.

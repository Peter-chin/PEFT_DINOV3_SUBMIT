...
from Datasets import ExcelDataset, FracAtlasDataset, InMemoryDataset, multi_class_preprocess, preload_dataset_to_ram_multiclasses
...
def main():
    ...
    if args.preload_to_ram:
        ...
        if args.datasets_type == "BTXRD":
            test_dataset = ExcelDataset_MultiTask(test_df, img_dir=img_dir, transform=tfr)
        elif args.datasets_type == "FracAtlas":
        ...
        for X_tv, y_tv, regime in regime_manager.get_data():
            ...
            for fold, (train_idx, val_idx) in enumerate(kf.split(X_tv, y_tv)):
                ...
                if n_prompt_tokens == 1 or n_prompt_tokens == 0:
                    ...
                    model = DINOv3LinearProbe(num_classes, repo_root, model_weight_path, full_finetune=args.full_finetune).to(device)
                else:
                    print(f"Using DINOv3 with {n_prompt_tokens} prompt tokens")
                    model = DINOv3PromptProbe(num_classes, repo_root, model_weight_path, n_prompt_tokens=n_prompt_tokens).to(device)
                # model = DINOv3LinearProbe(num_classes, repo_root, model_weight_path, full_finetune=args.full_finetune).to(device)
                if args.datasets_type == "BTXRD":
                    if args.preload_to_ram:

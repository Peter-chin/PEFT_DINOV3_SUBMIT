...
class DataRegime:
    ...
    def __init__(self, x_data, y_data, data_regime: str):
        ...
        self.regime_seeds = {'5%': 5, '10%': 10, '50%': 50}
        self.available_regimes = list(self.regime_seeds.keys())
    ...
    def get_data(self):
        ...
        if self.data_regime == 'all':
            # yield all available regimes in order
            for regime in self.available_regimes:
                x_sub, y_sub = self._subsample(regime)
                yield x_sub, y_sub, regime
            x_sub, y_sub = self.x_data, self.y_data
            yield x_sub, y_sub, self.data_regime  # yield the full dataset at the end
...
class SegDataRegime:
    """Subsampling manager for segmentation regimes using indices only.

    This class is designed for segmentation workflows where x_data is usually
    an array of dataset indices and y_data may be unavailable.
    """

    def __init__(self, x_data, data_regime: str, seed: int = 42):
    ...
    def _subsample(self, regime_str: str):
        regime = regime_str.strip().lower()
        ...
        seed = self.regime_seeds.get(regime, self.seed)
        rng = np.random.RandomState(seed)

        n_samples = len(self.x_data)
        n_select = max(1, int(np.ceil(n_samples * fraction)))
        indices = rng.choice(n_samples, size=n_select, replace=False)
        indices = np.sort(indices)

        return self.x_data[indices]

    def get_data(self):
        """Yield (x_subset, None, regime) just like the original interface."""
        if self.data_regime == 'all':
            for regime in self.available_regimes:
                x_sub = self._subsample(regime)
                yield x_sub, None, regime
        else:
            x_sub = self._subsample(self.data_regime)
            yield x_sub, None, self.data_regime

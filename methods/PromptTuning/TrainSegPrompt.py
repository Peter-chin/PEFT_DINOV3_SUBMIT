...
# segmentation task
from BaselineLinear import DINOv3PromptProbe_Seg
# split dataset
...
def grid_search(args, learning_rate_list):
    print(f"Running grid search over {len(learning_rate_list)} learning rates: {learning_rate_list}")

    grid_root = os.path.join(os.path.abspath(os.path.expanduser(args.result_root)), "grid_search")
    ...
    for lr in learning_rate_list:
        ...
        for regime, fold_metrics in results.items():
            for fm in fold_metrics:
                all_test_loss.append(fm["test_loss"])
                all_test_dice.append(fm["test_dice"])
                all_test_iou.append(fm["test_iou"])
                all_test_pixacc.append(fm["test_pixacc"])

        grid_summary[str(lr)] = {
            "mean_test_loss": float(np.mean(all_test_loss)) if all_test_loss else None,
            "mean_test_dice": float(np.mean(all_test_dice)) if all_test_dice else None,
            "mean_test_iou": float(np.mean(all_test_iou)) if all_test_iou else None,
            "mean_test_pixacc": float(np.mean(all_test_pixacc)) if all_test_pixacc else None,
        }

    summary_path = os.path.join(grid_root, "grid_search_summary.json")
...
def run_training(args, current_lr, result_root_override=None):
    ...
    print(f"\n===== Running prompt-token setting: {args.n_prompt_tokens} =====")
    ...
    for regime in regimes:
        ...
        for X_tv, _, regime_name in regime_items:
            ...
            all_results[regime] = fold_metrics
...
def main():
    ...
    parser.add_argument("--n-prompt-tokens", type=int, default=4, help="Number of prompt tokens")
    args = parser.parse_args()

    learning_rate_list = parse_float_list(args.learning_rate)
    if len(learning_rate_list) == 0:
        raise ValueError("No valid learning rate provided.")

    if len(learning_rate_list) > 1:
        grid_search(args, learning_rate_list)
    else:
        run_training(args, current_lr=learning_rate_list[0])

...
def train_one_epoch(model, loader, optimizer, criterion, device, desc=None, use_mixup=False, mixup_alpha=0.2):
    model.train()
    total_loss = 0
    correct = 0
    total = 0
    ...
    for x, y in tqdm(loader, desc=desc, leave=False, dynamic_ncols=True, mininterval=10):
        ...
        if use_mixup and mixup_alpha > 0 and x.size(0) > 1:
...
def save_best_epoch_summary_csv(save_dir, fold_results):
    ...
    per_fold_df = pd.DataFrame([
        {
            ...
            "best_epoch_by_val_auc": r.get("best_epoch_by_val_auc", 1),
            "best_val_auc": r.get("best_val_auc", 0.0),
            ...
            "test_auc_at_best_val_auc": r.get("test_auc_at_best_val_auc", 0.0),
            "test_acc_at_best_val_auc": r.get("test_acc_at_best_val_auc", 0.0),
            "test_loss_at_best_val_auc": r.get("test_loss_at_best_val_auc", 0.0),
            "test_ap_at_best_val_auc": r.get("test_ap_at_best_val_auc", 0.0),
            "test_f1_at_best_val_auc": r.get("test_f1_at_best_val_auc", 0.0),
            "test_bal_at_best_val_auc": r.get("test_bal_at_best_val_auc", 0.0),
        }
    ...
    summary_row = {
        ...
        "mean_best_val_auc": float(per_fold_df["best_val_auc"].mean()),
        "mean_best_val_acc": float(per_fold_df["best_val_acc"].mean()),
        "mean_best_val_loss": float(per_fold_df["best_val_loss"].mean()),
        "mean_test_auc_at_best_val_auc": float(per_fold_df["test_auc_at_best_val_auc"].mean()),
        "mean_test_acc_at_best_val_auc": float(per_fold_df["test_acc_at_best_val_auc"].mean()),
        "mean_test_loss_at_best_val_auc": float(per_fold_df["test_loss_at_best_val_auc"].mean()),
        "mean_test_ap_at_best_val_auc": float(per_fold_df["test_ap_at_best_val_auc"].mean()),
        "mean_test_f1_at_best_val_auc": float(per_fold_df["test_f1_at_best_val_auc"].mean()),
        "mean_test_bal_at_best_val_auc": float(per_fold_df["test_bal_at_best_val_auc"].mean()),
    }
...
def save_results(save_dir, fold_results):
    ...
    accs = [r.get("val_acc", 0.0) for r in fold_results]
    aucs = [r.get("val_auc", 0.0) for r in fold_results]
...
def main():
    ...
    for X_tv, y_tv, regime in regime_manager.get_data():
        ...
        if args.datasets_type == "BTXRD":
            # regime_dataset = ExcelDataset(df.iloc[X_tv].reset_index(drop=True), img_dir=img_dir, transform=train_tfr, cache_images=args.cache_dataset)
            ...
            # regime_dataset = FracAtlasDataset(df.iloc[X_tv].reset_index(drop=True), img_dir=img_dir, transform=train_tfr, cache_images=args.cache_dataset)
        ...
        for fold, (train_idx, val_idx) in enumerate(kf.split(regime_indices, regime_y)):
            ...
            model = DINOv3LinearProbe(2, repo_root, model_weight_path).to(device)
            # 印出 parameter 數量
            ...
            best_fold_val_auc = safe_get(val_aucs, best_idx_by_val_auc, 0.0)
            ...
            best_fold_val_bal = safe_get(val_bals, best_idx_by_val_auc, 0.0)
